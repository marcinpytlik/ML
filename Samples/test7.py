...
# --- 2. PREDICTIVE MAINTENANCE ---
    ...
    st.title("🏭 Wykrywanie Awarii z Wizualizacją Gaussa")
    st.write("Model dopasowuje krzywą dzwonową do każdej klasy danych.")

    # 1. Dane treningowe (Temperatura, Wibracje)
    X_train = np.array([[60, 2], [65, 3], [70, 2], [110, 8], [115, 9], [120, 10], [68, 4]])
    y_train = np.array([0, 0, 0, 1, 1, 1, 0]) # 0: OK, 1: Awaria
    ...
    with col1:
        ...
        temp = st.slider("Temperatura (°C)", 40, 140, 70)
        vib = st.slider("Wibracje", 0, 15, 3)
        
        prob = model.predict_proba([[temp, vib]])[0][1]
        st.metric("Prawdopodobieństwo awarii", f"{prob*100:.1f}%")

    with col2:
        st.subheader("Rozkład Gaussa dla Temperatury")
        
        # Wyciągamy parametry nauczone przez model (średnie i wariancje)
        # model.theta_ -> średnie (means)
        # model.var_   -> wariancje (variances)
        
        mean_ok = model.theta_[0][0]
        std_ok = np.sqrt(model.var_[0][0])
        
        mean_fail = model.theta_[1][0]
        std_fail = np.sqrt(model.var_[1][0])

        # Generujemy punkty na osi X dla wykresu
        x_axis = np.linspace(40, 140, 200)
        
        # Rysujemy wykres
        fig, ax = plt.subplots()
        ax.plot(x_axis, norm.pdf(x_axis, mean_ok, std_ok), label='Maszyna OK', color='green', lw=2)
        ax.plot(x_axis, norm.pdf(x_axis, mean_fail, std_fail), label='Awaria', color='red', lw=2)
        
        # Dodajemy linię aktualnego odczytu użytkownika
        ax.axvline(temp, color='blue', linestyle='--', label='Twój odczyt')
    ...
    if prob > 0.5:
        st.error("⚠️ UWAGA: Parametry wskazują na wysokie ryzyko awarii!")
    else:
        st.success("✅ Parametry w bezpiecznym zakresie.")
